...
<div class="col-12 pl">
	...
	<div class="product-items">
		...
		<div class="row">
		<?php		$counter=$pagenumber;
		while ($row_product=pdo_fetch($query))  {//			var_dump($row_product);
			$counter++;
...
//				$PriceFinal = "<span class='amount'>$price2<span>$price_unit</span></span>";
				$PriceTag = "
				<del><span>$price1  $price_unit </span></del><BR>
				<ins><span>$price2  $price_unit </span></ins>
				";
...
//				$PriceFinal = "<span class='amount'>$price1<span>$price_unit</span></span>";
				...
				<ins><span>$price1 $price_unit </span></ins>
				";
			...
			$TagBuy = "
			<li class='action-item add-to-cart' id='AddCart_$id' $TagBuyVisiblity>
			<button class='btn btn-light' title='افزودن به سبد خرید' onclick='AddToCart($id,$(\"#ProductProce$id\").val(),1);'>
			...
			if ($TagListPrice != "") {
				$TagListPrice = "<div class='price-list' $TagDivPrice ><select id='ProductProce$id' name='ProductProce$id' class='form-control' onchange='UpdatePrice($id);' >$TagListPrice</select></div>";
			}
			//-------------------- schema array 
			...
			<div class='col-lg-3 col-md-3 col-xs-12 col-6 order-1 d-block mb-3'>
				<section class='product-box product product-type-simple' style='min-height:395px;'>
					<div class='thumb' style='text-align:center;'>
						$StrSpectioal
						<div>$StrVote</div>
						<div class='text-right discount-d' id='darsad_$id'>$StrDarsad</div>
					...
					$TagListPrice
					<div class='price-list' id='PriceList_$id'>$PriceTag</div>
					<div class='actions' >
						<ul >
...
function UpdatePrice(id) {
	...
	let darsad = '';
	let StrDarsad='';
	let price_unit = '<?php echo $price_unit;?>';
	if (price2>0) {
		darsad = Math.round(100 - ((price2 * 100) / price1));
		if (darsad>0) StrDarsad= '<span>'+darsad+' %</span>'; else StrDarsad='';
		PriceTag = '<del><span>'+price1+'  '+price_unit+' </span></del><BR><ins><span>'+price2+'  '+price_unit+' </span></ins>';
	}else{
		PriceTag = '<del><span> &nbsp;</span></del><BR><ins><span>'+price1+'   '+price_unit+' </span></ins>';
	}	
	if ( cash<=0 ) {
		PriceTag = "<span class='btn btn-danger'>ناموجود</span>";
