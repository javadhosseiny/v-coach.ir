<?php
//variable 	 $ttime_change = number for change time server from iran (base=8.30 hour (NewYork)
//متغير $ttime_change جهت بدست آوردن تفاوت زمان سرور با ايران است 
// که هم اکنون با اختلاف 8.5 ساعت و با توجه به مکان سرور که نيويورک هست انجام مي گيرد

//
// +----------------------------------------------------------------------+
// | Persian Date Class													  |	
// +----------------------------------------------------------------------+
// | Copyright (c) 2007-2008 The Milad Jafary                             |
// +----------------------------------------------------------------------+
// | This source file is available at through the world-wide-web at       |
// | http://milad.jafary.googlepages.com/                                 |
// +----------------------------------------------------------------------+
// | Authors: Milad Jafary <milad.jafary@gmail.com>                       |
// +----------------------------------------------------------------------+
// 
class DateArabic
{
	...
	function  DateArabic($date='')
	{
		...
		{	
			...
			list($this->year,$this->month,$this->day)= $this->gregorian_to_jalali($year ,$month ,$day);		
		}
	}
    ...
    */
	function  DateNow()
    {
 		$ttime_change = time() ; // +( (8 * 60*60) + 30*60);
 		$ttime_change2 = time();// + (24 * 60*60) ; // +( (8 * 60*60) + 30*60);

		list($this->year,$this->month,$this->day)= $this->gregorian_to_jalali(date("Y",$ttime_change) ,date("m",$ttime_change) ,date("d",$ttime_change2));		
    ...
    */
	...
	{
 		$ttime_change = time() ; // +( (8 * 60*60) + 30*60);
		$this->hour=date("H",$ttime_change );
 		$this->minute=date("i",$ttime_change);
		$this->second=date("s",$ttime_change);
	}
   ...
   /**
    ...
    {
	  $week_names=array('‫السبت','‫الأحد','‫الاثنين'
						  ,'‫الثلاثاء'  ,'‫الأربعاء'
						  ,'‫الخميس' ,'‫الجمعة');
   ...
   /**
    ...
    {
	$month_names=array('محرم','صفر','ربیع الاول' 
   ...
   /**
	...
	// در حقیقت نام تابع می بایستی gregorian_to_jalali باشد 
	// ولیکن به دلیل تمایل به عدم اصلاح موارد دیگر محتوای تبدیل تاریخ میلادی به قمری در همین جا گذاشته می شود
