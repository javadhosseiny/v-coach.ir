...
class Date
{
    ...
    */
	function  DateNow()
    {
		...
		list($this->year,$this->month,$this->day)= $this->gregorian_to_jalali(date("Y",$ttime_change) ,date("m",$ttime_change) ,date("d",$ttime_change));		
   ...
   /**
    ...
    {
		$week_names=array('شنبه','يکشنبه','دوشنبه'
						  ,'سه شنبه'  ,'چهار شنبه'
						  ,'پنج شنبه' ,'جمعه');
   ...
   /**
    ...
    {
	$month_names=array('فروردين','ارديبهشت','خرداد'
						   ,'تير'   ,'مرداد'   ,'شهرويور'
						   ,'مهر'   ,'آبان'    ,'آذر'
						   ,'دي'    ,'بهمن'     ,'اسفند');
   ...
   /**
	...
	function jalali_to_gregorian($j_y, $j_m, $j_d)
	{
		$g_days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
		$j_days_in_month = array(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29);
		if (!is_numeric($j_y))  $j_y = intval($j_y);
		if (!is_numeric($j_m))  $j_m = intval($j_m);
		if (!is_numeric($j_d))  $j_d = intval($j_d);
	   $jy = $j_y-979;
	   $jm = $j_m-1;
	   $jd = $j_d-1;
