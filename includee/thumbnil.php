...
if (isset($_REQUEST['f'])) {  $save_to_file = intval($_REQUEST['f']) == 0;}

if (isset($_REQUEST['src'])) {  $from_name = urldecode($_REQUEST['src']);}
else {  die("Source file name must be specified.");}

//if (file_exists($from_name)) { $content = file_get_contents($from_name); echo $content; exit;}


if (isset($_REQUEST['dest'])) {  $to_name = urldecode($_REQUEST['dest']);}
else if ($save_to_file) {  die("Thumbnail file name must be specified.");}
...
if (isset($_REQUEST['scale'])) {  $scale = intval($_REQUEST['scale']);}

if (isset($_REQUEST['s'])) {  $size = intval($_REQUEST['s']);} else {$size=0;}// سطح سایز طول و عرض به صورت گروهی
if ($size==0) $size=1;

switch($size) {
     case 1: $max_x = 300;  $max_y = 440;   break;
     case 2: $max_x = 300;  $max_y = 200;   break;
}
...
//    if (substr($from_name,0,7) == 'http://') {
    if (filter_var($from_name, FILTER_VALIDATE_URL) ) {
