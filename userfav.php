...
$fieldsort1 = array("$bank1.countdn desc", "$bank1.nazarcount desc", " ($bank1.vote_sum / $bank1.vote_count) DESC", "$bank1.pdate desc, $bank1.ptime desc", "$bank2.price1", "$bank2.price1  desc ", "$bank1.best_saller desc", "$bank1.spectioal desc" ." ($bank2.price1-$bank2.price2) desc  " ); 
$fieldsort2 = array('پربازدیدترین',  'پربحث ترین', 'محبوب ترین', 'جدیدترین',  'ارزانترین', 'گرانترین', 'محصولات پرفروش', 'محصولات ویژه', 'بیشترین تخفیف' ); 

if ( ($scope<0)		and ($scope > (count($fieldlist1)-1) ) ) $scope=0;
if ( ($typesort<0) 	and ($typesort > (count($fieldsort1)-1) ) ) $typesort=0;
$sortquery = " order by " . $fieldsort1[$typesort] ;

$shart = " where $bank1.active=2 $shartdatetime and ";
...
$ShartFav = " and 1=1 ";
if ($ok_cookie) {
	$query=pdo_query("select id_product from `user_fav` where id_user='$usernameid' ");
	$rowall=pdo_fetchAll($query);
	$countall = pdo_rowcount($query);
	if ($countall==0) {
		$ErrorMsg = "بخش علاقه مندی ها شما کاربر گرامی [$usernamefarsi] خالی میباشد";
	}
	$rowall2 = array_column($rowall, 'id_product');
	$ListId	 = implode(',',$rowall2);
	$ShartFav = " and  ($bank1.id in ($ListId) )";
}
//------------------------
include('top.php');
...
if ($pagesize==0) { 
	$countrec = 1000;//$ListCountRec[0];
	$pagesize = $countrec; 
} else { 
	$countrec = $pagesize; 
}
$OkVote = true;
if ($row_setting['ok_save_vote']==1) {
	if (!$ok_cookie) $OkVote= false;
}
$javacode='';
/*
...
<div class='row container-main'>
	<div class='d-block'>
	<!-- search section  -->
	<div class='col-12 archive-header pr mt-1 d-none'>
...
//			var_dump($row_product);
			...
			$TagBuy = "<li class='action-item add-to-cart'><button class='btn btn-light' title='افزودن به سبد خرید' onclick='AddToCart($id,$id_price,1);'><i class='fa fa-shopping-cart'></i></button></li>";
			if ( $cash<=0 ) {
